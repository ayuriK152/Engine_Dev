...
void Geometry::EndCreateGeometry()
{
	...
	const UINT ibByteSize = (UINT)_indices.size() * sizeof(UINT16);
	...
	this->indexBufferGPU = DXUtil::CreateDefaultBuffer(GRAPHIC->GetDevice().Get(),
		GRAPHIC->GetCommandList().Get(), _indices.data(), ibByteSize, this->indexBufferUploader);
	...
	vector<Vertex>().swap(_vertices);
	vector<UINT16>().swap(_indices);
}
