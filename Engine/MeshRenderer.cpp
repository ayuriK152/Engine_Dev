...
void MeshRenderer::Render()
{
	...
	for (int i = 0; i < submeshes.size(); i++)
	{
		...
		cmdList->IASetVertexBuffers(0, 1, &submeshes[i]->vertexBufferView);
		cmdList->IASetIndexBuffer(&submeshes[i]->indexBufferView);
		cmdList->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

		CD3DX12_GPU_DESCRIPTOR_HANDLE tex(RENDER->GetShaderResourceViewHeap()->GetGPUDescriptorHandleForHeapStart());
		tex.Offset(_submeshMaterials[i]->diffuseSrvHeapIndex, GRAPHIC->GetCBVSRVDescriptorSize());
		...
		cmdList->DrawIndexedInstanced(submeshes[i]->GetIndexCount(), 1, 0, 0, 0);
	}
}
...
{
	_mesh = mesh;
	_submeshMaterials.clear();
	auto submeshes = _mesh->GetSubmeshes();
	for (int i = 0; i < submeshes.size(); i++)
	{
		_submeshMaterials.push_back(submeshes[i]->GetMaterial());
	}
}
